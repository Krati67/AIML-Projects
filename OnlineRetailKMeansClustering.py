...
country_cust_data=onlineRetail[['Country','CustomerID']].drop_duplicates()
...
onlineRetail = onlineRetail.query("Country=='United Kingdom'").reset_index(drop=True)
...
onlineRetail = onlineRetail[pd.notnull(onlineRetail['CustomerID'])]

#Validate if there are any negative values in Quantity column
onlineRetail.Quantity.min()
...
RFMScores = onlineRetail.groupby('CustomerID').agg({'InvoiceDate': lambda x: (Latest_Date - x.max()).days, 'ï»¿InvoiceNo': lambda x: len(x), 'TotalAmount': lambda x: x.sum()})
